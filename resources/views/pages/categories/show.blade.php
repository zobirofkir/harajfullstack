<x-app-layout title="{{ $category->title }}">
    <div class="container mx-auto px-4 py-8 h-screen">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Cars in {{ $category->title }}</h1>
            ...
            @forelse ($cars as $car)
                <a href="{{ route('cars.show', $car->slug) }}">
                    <div class="border border-gray-300 rounded-lg shadow hover:shadow-lg p-4 bg-white">
                        ...
                        <p class="mt-2 text-gray-900 font-bold">Price: {{ $car->price }} $</p>
                        <p class="text-sm text-gray-500 mt-1">Created at: {{ $car->created_at->format('M d, Y') }}</p>
                    </div>
                </a>
            @empty
